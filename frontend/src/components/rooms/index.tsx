...
export const ChatRoom: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    await fetch(ROOM_URL, requestOptions)
      .then(handleResponse)
      .then((response) => {
        toast.success(`Deleted the '${selectedRoom.room}' user successfully!`);
        dispatch(setRoom({}));
        navigate('/');
        return;
      });
    ...
    <Theme>
      <div className={style['content']}>
        <div className={style['chat-nav']}>
          {connectedUser &&
            connectedUser.map((username, key) => <UserItem key={key} label={username} />)}
        </div>
        <div className={style['chat-content']}>
          <div className={style['chat-head']}>
            <span>{selectedRoom.room}</span>
            <div>
              <Button type="button" variant="contained" onClick={handleLeave}>
                Leave
              </Button>
              {selectedRoom && selectedRoom.owner == currentUser.id && (
                <Button type="button" sx={{ ml: 3 }} variant="contained" onClick={handleDelete}>
                  Delete
                </Button>
              )}
            </div>
          ...
          <div className={style['chat-body']}>
            {messages && messages.map((message, key) => <MessageItem key={key} item={message} />)}
          </div>
          <div className={style['chat-footer']}>
            <input className={style['message-input']} value={text} onChange={onChnage} />
  ...
  return (
    <React.Fragment>
      <div className={style['message-item']}>
        <Avatar name={sender} size={'30px'} round={true} />
